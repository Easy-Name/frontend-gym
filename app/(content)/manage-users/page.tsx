...
export default function CRUDPage() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const response = await axios.get("172.40.3.140:3005/users/me", {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.patch(
        `172.40.3.140:3005/users/me/${selectedUserId}`,
        { ...formData },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`172.40.3.140:3005/users/me/${selectedUserId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
